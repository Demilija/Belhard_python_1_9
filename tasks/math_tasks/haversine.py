...
def distance(a_lat, a_lon, b_lat, b_lon):
    ...
    a_lat_sin = math.sin(a_lat_rad)
    b_lat_sin = math.sin(b_lat_rad)

    a_lat_cos = math.cos(a_lat_rad)
    b_lat_cos = math.cos(b_lat_rad)

    delta = a_lon_rad - b_lon_rad
    delta_sin = math.sin(delta)
    delta_cos = math.cos(delta)

    y = (math.sqrt(pow(b_lat_cos * delta_sin, 2))
         + math.sqrt(pow(a_lat_cos * b_lat_sin - a_lat_sin * b_lat_cos * delta_cos, 2)))
